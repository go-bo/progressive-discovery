...
const NavDivider = styled.div`
  height: 1px;
  margin: 12px ${({ theme }) => (theme as StyledTheme).space200};
  background-color: ${({ theme }) => (theme as StyledTheme).colorOutlineVariant};
  flex-shrink: 0;
`;

const PlatformSectionWrapper = styled.div`
  margin-top: auto;
`;
...
const NudgeBanner = styled.button`
  display: flex;
  align-items: center;
  gap: ${({ theme }) => (theme as StyledTheme).space200};
  margin: ${({ theme }) => `0 ${(theme as StyledTheme).space200} 0`};
  padding: ${({ theme }) => (theme as StyledTheme).space200};
  ...
  width: calc(100% - ${({ theme }) => (theme as StyledTheme).space200} - ${({ theme }) => (theme as StyledTheme).space200});
...
const NudgeIconCircle = styled.div`
  width: 28px;
  height: 28px;
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <StyledSidebar theme={theme} isCollapsed={isCollapsed}>
      <div>
        ...
        {mainSections.map((section, index) => (
          <React.Fragment key={`main-section-${index}`}>
            {section.dividerBefore && <NavDivider theme={theme} />}
            <NavSection 
              section={section} 
      ...
      {/* Platform Section - floated to bottom */}
      {platformSection && (
        <PlatformSectionWrapper theme={theme}>
          <NavSection 
            section={platformSection} 
            isCollapsed={isCollapsed} 
            theme={theme}
            onHelpClick={onOpenHelpPanel}
            afterLabel={
              nudge && !nudgeDismissed && !isCollapsed ? (
                <NudgeBanner theme={theme} onClick={nudge.onClick}>
                  <NudgeIconCircle theme={theme}>
                    <Icon type={nudge.icon} size={16} color={theme.colorOnPrimaryContainer} />
                  </NudgeIconCircle>
                  <NudgeText theme={theme}>{nudge.text}</NudgeText>
                  <NudgeDismiss
                    theme={theme}
                    onClick={(e) => {
                      e.stopPropagation();
                  ...
                  >
                    <Icon type={Icon.TYPES.CLOSE} size={12} color={theme.colorOnSurfaceVariant} />
                  </NudgeDismiss>
                </NudgeBanner>
              ) : undefined
            }
