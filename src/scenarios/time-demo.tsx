...
const TIME_DEMO_PAGES = new Set([
  'time-overview', 'payroll', 'platform-overview', 'app-shop',
  'data-overview', 'tools-overview', 'company-settings-overview',
  ...Object.keys(PLATFORM_PAGES),
]);

const DEFAULT_PAGE = 'time-overview';

const TimeDemo: React.FC = () => {
  const { page } = useParams<{ page?: string }>();
  const navigate = useNavigate();
  const [activePage, setActivePageState] = useState(() =>
    page && TIME_DEMO_PAGES.has(page) ? page : DEFAULT_PAGE,
  );
  ...
  // Sync from URL when it changes (e.g. back/forward, direct nav)
  useEffect(() => {
    if (!page || page === '') {
      setActivePageState(DEFAULT_PAGE);
    } else if (TIME_DEMO_PAGES.has(page) && page !== activePage) {
  ...
  const setActivePage = (pageId: string) => {
    ...
    if (pageId === DEFAULT_PAGE) {
      ...
      navigate(`/time/${pageId}`, { replace: true });
    }
  };
